...
import SkillsAndPortfolio from "../components/main/SkillsAndPortfolio";

const Main = () => {
  const resumeRef = useRef();
  const aboutMe = useRef();
  const skillsAndPortfolio = useRef();
  ...
  useEffect(() => {
    ...
    observer2.observe(resumeRef.current);
    observer3.observe(skillsAndPortfolio.current);
  ...
  return (
    <Wrapper>
      ...
      <SkillsAndPortfolio ref={skillsAndPortfolio} />
      <Contact ref={contact} />
      <Footer />
    </Wrapper>
  );
